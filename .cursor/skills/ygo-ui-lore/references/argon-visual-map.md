...
| Hub overlays | Compact quiet sheet, map visible | `HubChrome` + `DualMenuPresenter` (`GetWindowAnchors`, `QuietFill`) |
...
- A full-screen dest-tile binder on the GPS map (overlays must use compact `GetWindowAnchors` + `QuietFill`).
