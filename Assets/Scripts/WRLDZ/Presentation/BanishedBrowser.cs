...
{
    /// <summary>
    /// Banished / RFG browser — cloned from <see cref="GraveyardBrowser"/>.
    /// Face-up shows art + text; face-down (scripted FD) stays a card back.
    /// Presentation-only stub — Dilbot wires open/pick into DuelUI.
    /// </summary>
    public class BanishedBrowser : MonoBehaviour
    {
        GameObject _root;
        Text _title;
        Text _index;
        ...
        public static BanishedBrowser Create(Transform canvasRoot)
        {
            var host = new GameObject("BanishedBrowser", typeof(RectTransform));
            host.transform.SetParent(canvasRoot, false);
            FloatingPanel.Stretch(host.GetComponent<RectTransform>());
            var pop = host.AddComponent<BanishedBrowser>();
        ...
        public void Show(IReadOnlyList<CardInstance> banished, CardDatabase db, bool playerSide,
            Action<CardInstance> onPick)
        {
            ...
            if (banished != null)
            {
                for (var i = banished.Count - 1; i >= 0; i--)
                    if (banished[i] != null)
                        _cards.Add(banished[i]);
            }
        ...
        public void Refresh(IReadOnlyList<CardInstance> banished)
        {
            if (!IsOpen) return;
        ...
        {
            var n = _cards.Count;
            var who = _playerSide ? "YOUR BANISHED" : "OPP BANISHED";
        ...
        void PaintFeature()
        {
            ...
            if (_name != null)
                _name.text = card.FaceUp ? (card.Name ?? "Card") : "Face-down";
            if (_typeLine != null)
            {
                if (!card.FaceUp)
                {
                    _typeLine.text = "Banished face-down";
                }
                else
                {
        ...
        void PaintStrip()
        {
            ...
            for (var i = 0; i < _cards.Count; i++)
            {
                var idx = i;
                var card = _cards[i];
                var go = new GameObject(card.FaceUp ? (card.Name ?? "rf") : "rf_fd",
        ...
        /// <summary>Face-up → art; face-down → card back (public info rule).</summary>
